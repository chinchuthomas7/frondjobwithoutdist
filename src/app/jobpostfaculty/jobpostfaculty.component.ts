...
@Component({
  selector: 'app-jobpostfaculty',
  templateUrl: './jobpostfaculty.component.html',
  styleUrls: ['./jobpostfaculty.component.css']
})
export class JobpostfacultyComponent implements OnInit {
...
job={ 'compname':'','jbtitle':'','regemail':'','stdate':'','nddate':'','qulfn': '[]', 'exp':'','passout':'','skill':'','info':'','companyinfo':''}
  ...
  ngOnInit(): void {
    ...
    registeredEmail:[this.job.regemail, [Validators.required,Validators.pattern("^[a-z0-9.%]+@[a-z0-9.-]+\.[a-z]{2,4}")]],
...
submitHandler() {
 ...
 let  jobpostsfaculty={...this.reactiveForm.value,qlfnItem,passItem,skillItem}
  console.log({...this.reactiveForm,qlfnItem,passItem,skillItem})


  this.jobportalservice.jobpostfaculty(jobpostsfaculty)
  .subscribe(
